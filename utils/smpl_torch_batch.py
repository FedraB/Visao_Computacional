...
class SMPLModel(Module):
    def __init__(self, device=None, model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.shapedirs = torch.from_numpy(np.array(params['shapedirs'])).to(dtype=self.data_type, device=self.device)
    ...
    def forward(self, betas, thetas, trans, gR=None, halpe=False):
...
def test_gpu(data_type=torch.float):
    ...
    model = SMPLModel(
        device=device,
        model_path='D:\BuzhenHuang_Programs\occlusion-master\data_utils/model_lsp.pkl',
        data_type=data_type,
        simplify=True
    )
    '''
    pose = torch.from_numpy(mean_pose) \
        .type(data_type).resize(1, 72).to(device)
    betas = torch.from_numpy(mean_shape) \
        .type(data_type).resize(1, 10).to(device)
    '''
    pose = torch.from_numpy(np.zeros((1, pose_size))) \
        .type(data_type).to(device)
    betas = torch.from_numpy(np.zeros((1, beta_size))) \
        .type(data_type).to(device)
    trans = torch.from_numpy(np.zeros((1, 3))).type(data_type).to(device)
    result, joints = model(betas, pose, trans)
    print(model.pose, model.shape)
    model.write_obj(result[0].detach().cpu().numpy(), 'mean.obj')
