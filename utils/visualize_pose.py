...
def show_poses(poses):
    ...
    joint_type = np.arange(14)
    joint_type = np.expand_dims(joint_type, 0).repeat(len(poses), axis=0)
    poses = np.concatenate((joint_type[:,:,np.newaxis], poses), axis=-1)
    ...
    for coords in poses:
        ...
        for c, limb in enumerate(Pose.LIMBS_HALPE_14):
            type_a, type_b = limb
            jas = [j for j in coords if int(j[0]) == type_a]  # all joints of type 'type_a'
            jbs = [j for j in coords if int(j[0]) == type_b]  # all joints of type 'type_b'
