...
class MS_HGNN_oridinary(nn.Module):
    ...
    def init_adj(self, num_ped, batch, mask):
        ...
        mask = mask.repeat(1, num_ped, 1)

        rel_rec = rel_rec * mask
        rel_send = rel_send * mask

        return rel_rec, rel_send

    def forward(self, h_states, mask):
        ...
        rel_rec, rel_send = self.init_adj(actor_num, batch, mask)
        # iter 1
...
class MS_HGNN_hyper(nn.Module):
    ...
    def viz_two_affinity(self, aff_map, aff_map1):
        
        aff_maps = aff_map.detach().cpu().numpy()
        aff_maps1 = aff_map1.detach().cpu().numpy()
        for im, im1 in zip(aff_maps, aff_maps1):
            ...
            im1 = self.convert_color(im1*255)
            # im = cv2.resize(im, dsize=None, fx=10, fy=10, interpolation=cv2.INTER_NEAREST)
            cv2.namedWindow('aff',0)
            cv2.resizeWindow('aff',int(im.shape[1]*ratio),int(im.shape[0]*ratio))
            cv2.imshow('aff',im)
            cv2.namedWindow('aff1',0)
            cv2.resizeWindow('aff1',int(im1.shape[1]*ratio),int(im1.shape[0]*ratio))
            cv2.imshow('aff1',im1)
            cv2.waitKey()
    ...
    def forward(self, h_states, corr, mask, viz=False):
        ...
        edge_hidden, collective = self.node2edge(curr_hidden, H, idx=0) 
        
        if viz:
            collective = collective / 70
            self.viz_two_affinity(collective, corr)
